...
# 4
'''
Сколько существует различных четырехзначных чисел записанных в восьмеричной системе счисления, 
в записи которых есть ровно две одинаковые цифры причем стоящие рядом?
'''
c = 0
for i in range(512, 4096):
    ...
    d = 0
    for j in range(3):
        if b[j] == b[j+1] and b[0] != b[3] and b[1] != b[3]:
            d += 1
    if d == 1:
        c += 1
        print(i, int(b, 8), b, c)
...
f = open("k7-0.txt")
s = f.readline().replace('A', ' ').replace('B', ' ')
...
Найдите максимальную длину цепочки вида EABEABEABE… (состоящей из фрагментов EAB, 
...
# 11
'''
Текстовый файл состоит не более, чем из 10^6 символов из набораA, B, C.
